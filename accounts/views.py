...
def registeruser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            send_activation_email(request, user, email_subject, email_template)
            messages.error(
...
def registerVendor(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid() and v_form.is_valid():
            ...
            send_activation_email(request, user, email_subject, email_template)
            messages.success(
                request, 'your restaurant has been registered successfully ! pls wait for approval')
            messages.error(
                request, "Your vendor has been registered successfully!")
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        user.save()
        messages.success(
            request, 'Congratulation !!! Your account has activated. ')
        return redirect('myAccount')
